...
def reduce_dim(embeddings: np.ndarray, method: str = "PCA", seed: int = 614, n_workers: int = 1):
    """
    Reduce the dimensionality of embeddings to 2D using PCA or t-SNE.

    Args:
        embeddings (np.ndarray): The input feature embeddings of shape (n_samples, n_features).
        method (str, optional): The dimensionality reduction method, either "PCA" or "TSNE". Defaults to "PCA".
        seed (int, optional): Random seed for reproducibility. Defaults to 614 :)
        n_workers (int, optional): Number of parallel workers for t-SNE. Defaults to 1.
